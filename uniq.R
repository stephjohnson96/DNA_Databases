...
uniq <- function(N,r,s){
  ...
  population.dups <- (population[duplicated(population[, -1]), ]) # -1, ID will not going to match
  num.population.dups <- (dim(population.dups)[1])   # dim returns [1] the number of rows and [2] the number of cols.  
